...
int aptx_decode_update(struct audec_state **adsp, const struct aucodec *ac, const char *fmtp)
{
	...
	if (!ads->dec)
	{
		warning("Cannot initialize aptX decoder.\n");
		err = ENOMEM;
		goto out;
	}
...
int aptx_decode_frm(struct audec_state *ads, int fmt, void *sampv, size_t *sampc, const uint8_t *buf, size_t len)
{
	size_t processed = 0;
	size_t written = 0;
	...
	*sampc = written / APTX_WORDSIZE;
	...
	if (processed != len)
		warning("aptX decoding stopped in the middle of the sample, dropped %u bytes\n", (unsigned int)(len - processed));
