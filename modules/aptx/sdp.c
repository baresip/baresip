...
int aptx_fmtp_enc(struct mbuf *mb, const struct sdp_format *fmt, bool offer, void *arg)
{
	(void)offer;
	(void)arg;
	...
	return mbuf_printf(mb, "a=fmtp:%s "
					   ...
					   fmt->id, APTX_VARIANT == APTX_VARIANT_HD ? "hd" : "standard", APTX_VARIANT == APTX_VARIANT_HD ? 24 : 16);
}
...
bool aptx_fmtp_cmp(const char *lfmtp, const char *rfmtp, void *arg)
{
	...
	*/
	...
	if (param_value(rfmtp, "bitresolution") != (APTX_VARIANT == APTX_VARIANT_HD ? 24 : 16))
		return false;
