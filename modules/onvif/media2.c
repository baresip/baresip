	...
	struct soap_child *gsu)
{
	...
	err |= soap_set_value_fmt(tmpc, "PT%dS", 0);
...
{
	int err = 0;
	struct soap_child *oc, *decc, *brlc, *srrc, *tmp;
 ...
 *
 */
static int deviceio_GetVideoSources_h(const struct soap_msg *msg,
 ...
 *
 */
static int deviceio_GetAudioSources_h(const struct soap_msg *msg,
	struct soap_msg **prtresp)
{
	int err = 0;
	struct soap_msg *resp;
 ...
 *
 */
static int deviceio_GetAudioOutputs_h(const struct soap_msg *msg,
	struct soap_msg **prtresp)
{
	int err = 0;
	struct soap_msg *resp;
...
int media_GetProfiles_h(const struct soap_msg *msg, struct soap_msg **prtresp)
{
	...
	if (soap_msg_add_ns_str_param(
		resp, str_pf_media_wsdl, str_uri_media2_wsdl) ||
		soap_msg_add_ns_str_param(
		resp, str_pf_schema, str_uri_schema)) {
		err = EINVAL;
		goto out;
	...
	b = soap_add_child(resp, resp->envelope, str_pf_envelope, str_body);
	gpc = soap_add_child(resp, b, str_pf_media2_wsdl,
	...
	struct soap_fault *f)
{
	...
	err |= media_add_streamsetup(msg, gsu);
	...
	struct soap_msg **prtresp)
{
	...
	if (0 == pl_strcmp(&b->ns->uri, str_uri_deviceio_wsdl)) {
		err = deviceio_GetAudioSources_h (msg, &resp);
		goto out;
	}
...
int media_CreateProfile_h(const struct soap_msg *msg, struct soap_msg **prtresp)
{
	...
	tokenc = soap_child_has_child(cpc, NULL, str_token);
	...
	if (tokenc)
		strncpy(np->name, tokenc->value.p,
