			 ...
			 struct auenc_param *prm, const char *fmtp)
{
	...
	st->frame_size = ac->srate * ac->ptime / 1000;
	st->channels = ac->ch;
			 ...
			 const struct aucodec *ac, const char *fmtp)
{
	...
	/* Decoder */
	if (fmtp) {
		info("speex: select mode %d.\n", *fmtp);
		mode = speex_lib_get_mode(*fmtp);
		if (mode)
			info("speex: mode %s channels %d.\n", mode->modeName,
			     st->channels);
	}
	else {
		mode = resolve_mode(ac->srate);
	}
	...
	srate = ac->srate;
	speex_decoder_ctl(st->dec, SPEEX_SET_SAMPLING_RATE, &srate);
	speex_decoder_ctl(st->dec, SPEEX_GET_FRAME_SIZE, &st->frame_size);
		...
		int fmt, const void *sampv, size_t sampc)
{
	...
	while (sampc > 0) {
		...
		/* Assume stereo */
		if (2 == st->channels) {
			speex_encode_stereo_int(sampvi,
						st->frame_size, &st->bits);
		}

		ret = speex_encode_int(st->enc, sampvi, &st->bits);
		if (1 != ret) {
		...
		bool marker, const uint8_t *buf, size_t len)
{
	...
	while (speex_bits_remaining(&st->bits) >= MIN_FRAME_SIZE) {
		...
		if (2 == st->channels) {
			speex_decode_stereo_int(&sampvi[i],
						st->frame_size,
						&st->stereo);
		}
			  ...
			  const uint8_t *buf, size_t len)
{
	...
	/* Silence */
	speex_decode_int(st->dec, NULL, sampvi);
	*sampc = n;
...
static struct aucodec speexv[] = {
	...
	/* Stereo Speex */
	{LE_INIT, NULL, "speex", 32000, 32000, 2, 2, 0, speex_fmtp_wb,
	 encode_update, encode, decode_update, decode, pkloss, NULL, NULL},
	{LE_INIT, NULL, "speex", 16000, 16000, 2, 2, 0, speex_fmtp_wb,
	 encode_update, encode, decode_update, decode, pkloss, NULL, NULL},
	{LE_INIT, NULL, "speex",  8000,  8000, 2, 2, 0, speex_fmtp_nb,
	 encode_update, encode, decode_update, decode, pkloss, NULL, NULL},

	/* Standard Speex */
	{LE_INIT, NULL, "speex", 32000, 32000, 1, 1, 0, speex_fmtp_wb,
	 encode_update, encode, decode_update, decode, pkloss, NULL, NULL},
	{LE_INIT, NULL, "speex", 16000, 16000, 1, 1, 0, speex_fmtp_wb,
	 encode_update, encode, decode_update, decode, pkloss, NULL, NULL},
	{LE_INIT, NULL, "speex",  8000,  8000, 1, 1, 0, speex_fmtp_nb,
	 encode_update, encode, decode_update, decode, pkloss, NULL, NULL},
};
