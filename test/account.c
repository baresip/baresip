...
int test_account_stun_uri_complete(void)
{
	...
	/* verify the STUN credentials from params */
	ASSERT_STREQ("bob@bob.com", account_stun_user(acc));
	ASSERT_STREQ("taj:aa", account_stun_pass(acc));
	ASSERT_STREQ("stunserver.org", account_stun_host(acc));
	...
	/* verify the STUN user from URI */
	ASSERT_STREQ("42", account_stun_user(acc));
	ASSERT_STREQ("taj:aa", account_stun_pass(acc));
	ASSERT_STREQ("stunserver.org", account_stun_host(acc));
	...
	/* verify the STUN user/password from URI */
	ASSERT_STREQ("42", account_stun_user(acc));
	ASSERT_STREQ("19", account_stun_pass(acc));
	ASSERT_STREQ("stunserver.org", account_stun_host(acc));
