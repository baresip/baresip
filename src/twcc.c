...
static void handle_chunks(struct twcc_status *twccst, bool run_finish)
{
	...
	if (run_finish && twccst->equal_state && status_cnt >= 7) {
		chunk = handle_run_chunk(twccst);
		 ...
		 twccst->max_state <= TWCC_PK_RECEIVED && status_cnt == 14) {
		...
		while (le) {
		 ...
		 twccst->max_state >= TWCC_PK_LARGE_DELTA && status_cnt == 7) {
...
static void send_feedback(void *arg)
{
	...
	while (le) {
		...
		if (p->ts && (p->delta < 0 || p->delta > UINT8_MAX))
			p->state = TWCC_PK_LARGE_DELTA;

		if ((twccst->msg.chunks->pos + twccst->msg.deltas->pos +
		     sizeof(twccst->msg)) >= TWCC_PKT_SIZE) {
			tmr_delay = 0;
			break;
		}
		...
		if (le) {
			struct rtcp_twcc_packet *p_next =
				mem_pool_member(le->data);
...
void twcc_status_append(struct twcc_status *twccst, uint16_t tseq, uint64_t ts)
{
	...
	if (rtp_seq_less(tseq, twccst->last_tseq)) {
